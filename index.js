...
server.tool("get-tokens-api-keys-credentials-from-store", "This tool will get the tokens, api keys, and credentials for the user from the store", {}, async () => {
...
async function generateAccessToken(url, key, credentialsJson, fullPath) {
...
async function refreshAccessToken(url, key, credentialsJson, fullPath, refreshToken) {
...
server.tool("get-auth-token", "Get an auth token for the user given auth url. This tool will check if token is already generated and saved to file. If not, it will generate a token and save it to file and return the token. If token is already generated and saved to file, it will return the token.", {
